...
from ..models import Question,Category,QuestionCount,Answer
...
def index(request, category_name='qna'):
    ...
    question_list = question_list.filter(category__name=category_name).distinct()
    # 정렬
    if so == 'recommend':
    ...
    if kw:
        ...
        question_list.filter(category__name=category_name).distinct()
...
def detail(request, question_id):
    ...
    context = {'question':question, 'answer_list': page_obj, 'so':so, 'page':page}
